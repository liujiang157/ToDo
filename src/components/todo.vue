...
<div>
	...
	<thead>
	<tr>
    <th><input v-model=searchinfo type="text" @keyup.enter="text" class="form-control"  placeholder="搜索"></th>
		<th><b-button v-model=searchinfo @click="searchbytext">搜索</b-button></th>
    ...
    <b-btn class="mt-3" variant="outline-danger" block @click="addItem"><a href="#">确定</a></b-btn>
  </b-modal>
  ...
  <table class="table table-striped">
	    ...
	    <tr v-for="(item,index) in items">
	        ...
	        <td v-if="!isupdate&&!item.isfinished">{{ item.date }}</td>
					<td v-if="isupdate" @keyup.enter="item.date=getDate(new Date(item.date));;isupdate=false"><datepicker  v-model="item.date" placeholder="点击选择时间" style="margin-left:120px"></datepicker></td>
					<td>
	        	<button class="btn btn-info" @click="isupdate='ture'">编辑</button>
	        </td>
	        <td>
	        	<button class="btn btn-danger" @click="deleteItem(item.text)">删除</button>
	        </td>
	    </tr>
	    </tbody>
	</table>
